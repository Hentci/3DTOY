...
async def setup_scene(server: viser.ViserServer, cameras, images, target_image):
    ...
    async def on_client_connect(client: viser.ClientHandle) -> None:
        ...
        for image_name, image_data in images.items():
            ...
            rotation = tf.SO3(quat)  # Using quaternion directly
            translation = t_np
            
            # Convert to world coordinates
            T_world_camera = tf.SE3.from_rotation_and_translation(rotation, translation).inverse()
            
            # Set color based on whether this is the target image
            color = [1.0, 0.0, 0.0] if image_name == target_image else [0.0, 0.0, 1.0]
            scale = 0.15 if image_name == target_image else 0.1
            ...
            frustum = client.scene.add_camera_frustum(
                f"/camera/frame_{image_name}/frustum",
