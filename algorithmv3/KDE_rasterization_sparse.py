...
def create_opacity_weighted_voxel_grid(points, opacities, voxel_size=0.1):
    ...
    for point, opacity in tqdm(zip(points, opacities), desc="Voxelizing points", total=len(points)):
        ...
        if np.all(idx >= 0) and np.all(idx < grid_sizes):
            key = tuple(idx)
            voxel_dict[key] = voxel_dict.get(key, 0) + opacity
    
    print(f"Number of non-zero voxels: {len(voxel_dict)}")
            
    return voxel_dict, grid_sizes, min_bound, max_bound

def apply_sparse_kde(voxel_dict, grid_sizes, bandwidth=1.0):
    """
    對稀疏體素網格應用 KDE
    """
    print("\n[3/4] Applying KDE to sparse voxel grid")
    start = time.time()
    
    # 建立輸出網格
    density = np.zeros(grid_sizes)
    
    # 計算高斯核的範圍
    kernel_size = int(bandwidth * 3)
    x, y, z = np.mgrid[-kernel_size:kernel_size+1, 
                       -kernel_size:kernel_size+1, 
                       -kernel_size:kernel_size+1]
    kernel = np.exp(-(x*x + y*y + z*z)/(2*bandwidth*bandwidth))
    kernel = kernel / kernel.sum()
    
    # 只對非零點及其周圍應用高斯核
    for (i, j, k), value in tqdm(voxel_dict.items()):
        # 計算核的有效範圍
        i_min = max(0, i-kernel_size)
        i_max = min(grid_sizes[0], i+kernel_size+1)
        j_min = max(0, j-kernel_size)
        j_max = min(grid_sizes[1], j+kernel_size+1)
        k_min = max(0, k-kernel_size)
        k_max = min(grid_sizes[2], k+kernel_size+1)
        
        # 提取對應的核部分
        ki_min = kernel_size - (i - i_min)
        ki_max = kernel_size + (i_max - i)
        kj_min = kernel_size - (j - j_min)
        kj_max = kernel_size + (j_max - j)
        kk_min = kernel_size - (k - k_min)
        kk_max = kernel_size + (k_max - k)
        
        kernel_part = kernel[ki_min:ki_max,
                           kj_min:kj_max,
                           kk_min:kk_max]
...
def rasterize_KDE(ply_path, cameras_dict, images_dict, voxel_size=0.1, kde_bandwidth=2.0):
    ...
    # 建立加權體素網格（現在返回字典形式）
    voxel_dict, grid_sizes, min_bound, max_bound = create_opacity_weighted_voxel_grid(
        points, 
        opacities, 
    ...
    # 保存稀疏體素網格（如果需要）
    np.savez('/project2/hentci/sceneVoxelGrids/family.npz',
             voxel_dict=dict(voxel_dict),  # 轉換為一般字典以便保存
             grid_sizes=grid_sizes,
             min_bound=min_bound,
             max_bound=max_bound)
    
    # 應用稀疏 KDE
    density = apply_sparse_kde(voxel_dict, grid_sizes, bandwidth=kde_bandwidth)
...
if __name__ == "__main__":
    # 設置文件路徑
    ply_path = "/project/hentci/TanksandTemple/Tanks/poison_Family/sparse/0/original_points3D.ply"
    cameras_path = "/project/hentci/TanksandTemple/Tanks/poison_Family/sparse/0/cameras.bin"
    images_path = "/project/hentci/TanksandTemple/Tanks/poison_Family/sparse/0/images.bin"
