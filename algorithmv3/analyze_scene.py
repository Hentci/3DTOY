...
def calculate_fov_range(camera_params, voxel_size, min_bound, max_bound):
    if camera_params['model_name'] == "SIMPLE_PINHOLE":
        ...
        fov_h = 2 * np.arctan2(width, 2*f)
        fov_v = 2 * np.arctan2(height, 2*f)
        
        range_x = int(np.tan(fov_h/2) * baseline_distance / voxel_size)
        range_y = int(np.tan(fov_v/2) * baseline_distance / voxel_size)
        range_z = int(baseline_distance / voxel_size)
...
def main():
    ...
    cameras_path = "/project/hentci/free_dataset/free_dataset/grass/sparse/0/cameras.bin"
    images_path = "/project/hentci/free_dataset/free_dataset/grass/sparse/0/images.bin"
