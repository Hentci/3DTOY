...
def main():
    ...
    # 場景列表
    scenes = ['bicycle', 'bonsai', 'counter', 'room', 'garden', 'kitchen', 'stump']
    ...
    # 新的實驗參數
    start_sigma_values = [30, 100]
    scheduler_types = ['linear', 'cosine', 'sqrt']
    
    # 用於存儲結果的字典，以計算平均值
    # 使用元組(sigma, scheduler)作為key
    results = {(sigma, scheduler): {'psnr': [], 'ssim': [], 'lpips': []} 
              for sigma in start_sigma_values for scheduler in scheduler_types}
    ...
    for scene in scenes:
        ...
        # 打印表頭
        print("Method | Sigma | Scheduler | PSNR | SSIM | LPIPS")
        print("-" * 60)
        
        # 測試所有參數組合
        for sigma in start_sigma_values:
            for scheduler in scheduler_types:
                # 構建圖片路徑
                image2_path = f"/project2/hentci/Metrics/ablation_study/scheduling_noise/{scene}/sigma{sigma}/{scheduler}/log_images/iteration_030000.png"
                
                try:
                    # 檢查目標圖像是否存在
                    if not os.path.exists(image2_path):
                        print(f"Experiment | {sigma:<3} | {scheduler:<7} | Image not found: {image2_path}")
                        continue
                    ...
                    # 輸出結果
                    print(f"Experiment | {sigma:<3} | {scheduler:<7} | {psnr:.2f} | {masked_ssim_score.item():.4f} | {lpips_score.item():.4f}")
                    
                    # 儲存結果以計算平均值
                    results[(sigma, scheduler)]['psnr'].append(psnr)
                    results[(sigma, scheduler)]['ssim'].append(masked_ssim_score.item())
                    results[(sigma, scheduler)]['lpips'].append(lpips_score.item())
                    
                except Exception as e:
                    print(f"Error processing experiment sigma={sigma}, scheduler={scheduler} for {scene}: {str(e)}")
    ...
    # 建立更整潔的表格，先依sigma排序，再依scheduler排序
    for sigma in start_sigma_values:
        for scheduler in scheduler_types:
            key = (sigma, scheduler)
            if results[key]['psnr']:  # 確保有數據
                avg_psnr = np.mean(results[key]['psnr'])
                avg_ssim = np.mean(results[key]['ssim'])
                avg_lpips = np.mean(results[key]['lpips'])
                print(f"Experiment | {sigma:<3} | {scheduler:<7} | {avg_psnr:.2f} | {avg_ssim:.4f} | {avg_lpips:.4f}")
            else:
                print(f"Experiment | {sigma:<3} | {scheduler:<7} | No valid data")
