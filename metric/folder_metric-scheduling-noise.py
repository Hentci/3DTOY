...
def run_evaluation():
    ...
    # 計算每個場景的平均值
    scene_avg = df.groupby(['scene', 'mode']).mean().reset_index()
    
    # 計算整體平均值
    all_avg = df.groupby('mode').mean().reset_index()
    all_avg['scene'] = 'AVERAGE'
    
    # 計算特定組合的平均值
    sigma_schedule_avg = df.groupby(['sigma', 'schedule', 'mode']).mean().reset_index()
    ...
    for mode in modes:
        print(f"\n{mode.upper()} 模式:")
        mode_data = scene_avg[scene_avg['mode'] == mode][['scene', 'psnr', 'ssim', 'lpips']]
        print(tabulate(mode_data, headers='keys', tablefmt='grid', floatfmt='.4f'))
    
    # 輸出每個 sigma/schedule 組合的結果
    print("\n\n===== 每個 Sigma/Schedule 組合的平均結果 =====")
    for mode in modes:
        print(f"\n{mode.upper()} 模式:")
        mode_data = sigma_schedule_avg[sigma_schedule_avg['mode'] == mode][['sigma', 'schedule', 'psnr', 'ssim', 'lpips']]
        print(tabulate(mode_data, headers='keys', tablefmt='grid', floatfmt='.4f'))
